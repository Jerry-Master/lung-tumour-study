...
def run_early_topo(args: Namespace, logger: Logger) -> None:
    ...
    newargs = Namespace(
        ...
        validation_node_dir=os.path.join(args.root_dir, 'data', 'validation', 'graphs', 'preds'),
        test_node_dir=os.path.join(args.root_dir, 'data', 'test', 'graphs', 'preds'),
        log_dir=os.path.join(args.output_dir, 'gnn_persistence_cubical_logs'),
        early_stopping_rounds=10,
        batch_size=10,
        model_name='GCN',
        save_file=os.path.join(args.output_dir, 'gnn_persistence_cubical_logs', 'gnn_results'),
        num_confs=32,
        save_dir=os.path.join(args.root_dir, 'weights', 'classification', 'gnn_persistence_cubical'),
        device=device,
        num_workers=args.num_workers,
        checkpoint_iters=-1,
        num_classes=args.num_classes,
