...
class GraphSAGE(nn.Module):
    def __init__(self, in_feats, h_feats, num_classes, num_layers, drop_rate, norm_type, enable_background=False):
        ...
        self.conv_layers.append(SAGEConv(in_feats, h_feats, 'pool', activation=F.elu))
        ...
        for l in range(1,num_layers):
            self.conv_layers.append(SAGEConv(h_feats, h_feats, 'pool', activation=F.elu))
            self.conv_layers.append(nn.Dropout(drop_rate))
            self.conv_layers.append(Norm(norm_type=norm_type, hidden_dim=h_feats))
        self.conv_layers.append(SAGEConv(h_feats, num_classes, 'pool'))

        self.enable_background = enable_background
        if enable_background:
            self.bkgr_head = SAGEConv(h_feats, 2, 'pool')
